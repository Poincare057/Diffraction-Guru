...
##    plt.show()
    ...
    for ity in range(k):
        for itx in range(k):
            ...
            phi_real[ity][itx] = V*f_real + ((-1)**random.randint(1, 10))*math.sqrt(abs(0.5*(f2_real - f_real**2)/n))
            phi_im[ity][itx] = V*f_im + ((-1)**random.randint(1,10))*math.sqrt(abs(0.5*(f2_im - f_im**2)/n))
